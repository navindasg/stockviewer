...
function validateForm(form: FormState): FormErrors {
  if (!form.tickerValidated) return { ticker: 'Select a valid ticker from search results' }
  const shares = parseFloat(form.shares)
  if (!form.shares || isNaN(shares) || shares <= 0) return { shares: 'Shares must be greater than 0' }
  const price = parseFloat(form.price)
  if (!form.price || isNaN(price) || price <= 0) return { price: 'Price must be greater than 0' }
  if (!form.date) return { date: 'Date is required' }
  if (new Date(form.date) > new Date()) return { date: 'Date cannot be in the future' }
  return {}
}

export function AddTransactionModal({ isOpen, onClose, prefillTicker }: AddTransactionModalProps) {
  const [form, setForm] = useState<FormState>(() => createInitialState(prefillTicker))
  ...
  useEffect(() => {
    if (isOpen) {
      setForm(createInitialState(prefillTicker))
      setErrors({})
      setSubmitError(null)
  ...
  return (
    ...
    >
      <div
        className="bg-sv-elevated rounded-lg w-full max-w-md mx-4 shadow-xl border border-sv-border"
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-sv-border">
          <h2 className="text-lg font-semibold text-sv-text">Add Transaction</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-4">
          ...
          <div>
            ...
            {form.type === 'SELL' && form.tickerValidated && (
